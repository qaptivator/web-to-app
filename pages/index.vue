  ...
  <div>
    ...
    <div class="m-auto p-5 mt-10">
      <div class="w-full text-center">
          <button class="border rounded-lg mt-5 py-2 px-10 bg-green-300 shadow-md" spellcheck="false" @click="getHash">get hash</button>
      </div>
...
import SmsRetreiver from 'capacitor-sms-retriever';
//let smsRetreiver = window['cordova']['plugins']['smsRetriever'];
export default {
  name: "IndexPage",
  data() {
    return {
  ...
  mounted() {
    ...
    SmsRetreiver.addListener('onSmsReceive', this.smsReceiver);
  },
  methods: {
    smsReceiver({message, error}) {
      alert("New sms: " + message + " new error: " + error)
    },
    getHash() {
      alert(SmsRetreiver.getAppSignature());
    },
    setUrl() {
